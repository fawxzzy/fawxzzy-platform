...
test('account broker rejects exact-denominator and pair-substitution regressions in both validators', () => {
  const cases = [
    ...
    ['unauthenticated issuance proof omitted', (contract) => { contract.verification.required_negative_probes[contract.verification.required_negative_probes.indexOf('UNAUTHENTICATED_EXCHANGE_ISSUANCE_REJECTED')] = 'ARBITRARY_NEGATIVE_PROBE'; }],
    ['caller-selected issuance subject proof omitted', (contract) => { contract.verification.required_negative_probes[contract.verification.required_negative_probes.indexOf('CALLER_SELECTED_EXCHANGE_SUBJECT_REJECTED')] = 'ARBITRARY_NEGATIVE_PROBE'; }],
    ...
    ['authorization code log-sink proof omitted', (contract) => { contract.verification.required_negative_probes[contract.verification.required_negative_probes.indexOf('AUTHORIZATION_CODE_LOG_SINK_NON_ECHOING')] = 'ARBITRARY_NEGATIVE_PROBE'; }],
    ...
    ['inherited session security proof omitted', (contract) => { contract.verification.required_positive_probes[contract.verification.required_positive_probes.indexOf('INHERITED_SESSION_SECURITY_READBACK_AND_RUNTIME_PROOF')] = 'ARBITRARY_POSITIVE_PROBE'; }],
