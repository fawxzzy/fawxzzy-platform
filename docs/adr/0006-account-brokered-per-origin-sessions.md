...
The broker relation belongs in `platform_private`, remains outside the Data API, enables and forces RLS as defense in depth, stores no plaintext code or session material, and exposes no function grants to `PUBLIC`, `anon`, or `authenticated`. Only the separately provisioned `platform_private_broker_runtime` server role receives helper `EXECUTE`; it is not a browser or Data API role. Preview proof must keep a credential-shaped synthetic canary out of every issuer, redemption, platform, analytics, and application log sink, and must prove a 64-character password survives signup or reset and later authentication unchanged. This repository includes no executable migration, secret, credential, provider link, or runtime configuration.
