...
The broker relation belongs in `platform_private`, remains outside the Data API, enables and forces RLS as defense in depth, stores no plaintext code or session material, and exposes no function grants to `PUBLIC`, `anon`, or `authenticated`. Only the separately provisioned `platform_private_broker_runtime` server role receives helper `EXECUTE`; it is not a browser or Data API role, and it derives exchange subjects only from a verified account session. Preview proof must keep credential-shaped and authorization-code-shaped synthetic canaries out of every issuer, redemption, callback, proxy, platform, analytics, and application log sink; prove a 64-character password survives signup or reset and later authentication unchanged; and read back plus exercise the inherited session lifetime, inactivity, rotation, and reuse-detection controls. This repository includes no executable migration, secret, credential, provider link, or runtime configuration.
