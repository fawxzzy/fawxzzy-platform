...
Data API state is held source contract, not a provider mutation. Contract v1.2.0 separates the observed current preimage from the required containment postimage: the target is currently enabled with ordered Exposed schemas `[graphql_public, public]`, ordered Extra search path `[public, extensions]`, automatic exposure off, tables 0/0, functions 1/0, and the view denominator still `UNKNOWN/UNKNOWN`; the required state is disabled with Exposed schemas `[]`, Extra search path `[extensions]`, and automatic exposure off. Two guarded Overview Save attempts did not persist, Settings Save was not attempted, rollback was unnecessary, persisted provider mutations remain zero, and the final state equals the observed preimage. Sanitized Support case `SU-425819` is current, but its case status and response timestamp remain `UNKNOWN`. Support observed the Data API enabled and requested one reproduction plus sanitized error-visual and network-activity-visual diagnostics. This does not confirm a provider defect, root cause, role change, alternate endpoint, or paid action.

The prior two-Save authority is consumed. FP-MAN-037 records operator approval for at most one guarded reproduction attempt and sanitized diagnostic capture, but this source contract grants no Dashboard or provider execution authority. A separately admitted provider-execution packet remains required, and no attempt has been executed under this ratchet. Screenshots, network bodies, headers, cookies, tokens, keys, request or response payloads, user data, PII, project secrets, and machine paths must never enter the source contract or public receipt.
