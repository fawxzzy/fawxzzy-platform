...
Core workout state uses complete key and canonical-row comparisons plus CAS. Inserts are parent-first and deletes are explicit, child-first tombstones. The nullable references between `routine_days` and `workout_plan_templates` form a declared foreign-key cycle, so rehearsal must insert both sides with the cycle references null and then patch the exact expected references through CAS; deterministic synthetic proof is required. `progression_events` preserves append-only identities. `exercise_stats` is excluded from transport and rebuilt only after authoritative workout parity.
