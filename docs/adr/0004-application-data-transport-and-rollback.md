...
`public.discord_member_links.fitness_user_id` is the relation's non-null, unique source owner key and references `auth.users.id`. Transport may rekey it only when `platform_private.source_identity_ledger` and the controlled Auth import mapping prove exactly one accepted source-to-target identity mapping. Missing, contradictory, or duplicate mappings, caller-selected identity, and any attempt to treat a Discord identifier as identity evidence all quarantine the record. The relation therefore remains `UNKNOWN_DISCORD_EXTERNAL` and held pending Discord identity adjudication; recognizing its Fitness owner key does not authorize Discord linkage, automatic identity merge, transport execution, or target writes.

Billing customers, purchases, entitlements, session follow-up jobs, and the Discord-named relations owned by the Fitness source are not silently imported or discarded. Billing and entitlement state waits for verified billing provenance and a closed adapter. Follow-up jobs remain quarantined as operational effects. Fitness-owned Discord identities, commands, Spotify/provider state, update drafts, verification tokens, and reporting state remain explicit `UNKNOWN`/held classes; Discord IDs, usernames, snapshots, and Fitness numbers cannot link or authorize a human. The historical `discord_bug_reports` identity is explicitly excluded as superseded by `discord_feedback_reports`.

The DiscordOS adapter is separately provider-canonical and source-ready. It binds the accepted 17-migration history, the 10-relation catalog, and the source/external-effect denominators without allowing the current 11-migration default branch to replace accepted paths or bytes. Five relations are authoritative mutable state, four are authoritative append-only history, and `discordos.discord_update_drafts` is held with no transport. The nine transportable relations require complete S0/S1/S2 key-set and canonical-row-digest parity, CAS against `ABSENT` or the exact target digest, explicit tombstones and resurrection, dependency-safe rollback, and aggregate-only receipts.

DiscordOS remains an operational-only service with no human profile, entitlement, or activation contract. A Discord ID, fingerprint, username, label, or snapshot is opaque external evidence and never a human identity key. Only an accepted identity-ledger mapping may authorize a rekey; missing, duplicate, or contradictory evidence quarantines the row and preserves any existing membership as pending. Synthetic proof rows are excluded and Fitness-semantic overlaps are quarantined rather than merged.

All public RPCs, the `pg_cron`/`pg_net` scheduler path, the network helper, Edge routing, credentials, aliases, provider links, webhooks, moderation actions, and Discord API writes remain blocked. The inert bootstrap continues to emit exactly nine DiscordOS relations, one trigger helper, five triggers, and 22 indexes, with zero extension, data, cron, or network effects. This source contract does not change generated artifacts or admit provider execution.

## Consequences

The generic contract remains app-agnostic. Mazer, Fitness, and DiscordOS now have closed source-ready adapters, but source readiness is not execution authority. Application-data execution remains blocked by Data API containment, accepted recovery and quarantined restore, faithful contained replay, target bootstrap, shared Auth identity mapping, Fitness PR #108 retirement, DiscordOS external-effect quarantine, and target apply. No source pause, reverse catch-up, target write, provider action, or deployment is admitted here.
