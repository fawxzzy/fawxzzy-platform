...
Storage object bodies are `NOT_APPLICABLE` only while independent, current, sanitized inventory evidence proves the exact aggregate bucket, object, and byte denominator is zero. That evidence is supplied separately from the backup receipt, binds the source project and source-state digest, provider Storage snapshot identity, backup snapshot, immutable export identity, observation time, and a deterministic manifest digest (including the canonical empty manifest), and must remain distinct from other evidence classes. A separately verified Ed25519 result authenticates the independent provider readback under `fawxzzy-platform:storage-inventory-attestation:v1`; its external receipt is the inventory source-readback digest, not a receipt-owned or recomputable manifest. The first non-zero object count, or any explicit claim of body coverage, additionally requires a separate closed Storage-body recovery receipt supplied at validation time. Supabase database backup evidence does not prove Storage body coverage and cannot by itself replace the independently authenticated inventory readback.
...
7. an immutable destination version plus a separate closed Object Lock readback and provider-bound signed attestation whose canonical identities match the exact bucket, object version, ciphertext digest, compliance mode, retention deadline, observation, and receipt;
8. a separate, closed, current accepted-exports manifest whose canonical digest matches the receipt, an external readback that proves the complete month-to-observation history, and an Ed25519-signed verifier result checked against the immutable public trust anchor in the versioned contract; retention is 35 days generally and 400 days for the export with the earliest unique `accepted_at` in the UTC month;
...
The accepted-exports manifest, closed history readback, and signed external authentication result are supplied independently from the receipt at validation time. A digest or `VERIFIED` label the claimant can recompute is correlation only and never authentication. The result binds an opaque external receipt digest, exact verifier and key identity, project/month/evidence identity, verification time, and the canonical history-readback digest calculated with `authentication.verification_result_sha256` omitted. Its signed bytes are deterministic and domain-separated as `fawxzzy-platform:accepted-exports-authentication:v1`, a NUL byte, and the canonical result metadata with `signature_base64`, `signed_payload_sha256`, and `result_sha256` omitted. The validator checks the signed-payload digest, result digest, canonical Ed25519 signature, public-key SPKI digest, key type, and exact key/verifier identity against the versioned trust anchor before accepting the result.

The committed source contract intentionally keeps the operational trust anchor `BLOCKED` with no public key. No backup receipt can pass that gate until a separately reviewed source packet pins the owned public Ed25519 SPKI and fingerprint. The signing private key is installed only through a later secure execution boundary and never enters Git, receipts, logs, or automation configuration. Tests use a published non-production RFC vector solely to exercise the cryptographic verifier.

Receipt validation also requires independently authenticated execution admission. `BACKUP_CURRENT` requires the source-owned `backup_generation_or_upload` gate at version `1.0.0` and `backup_receipt` scope. `RESTORE_REHEARSED` requires that admission plus `restore_rehearsal` at version `1.0.0` and `restore_receipt` scope. Before execution, each admission binds the canonical contract and source policy artifact digests, exact project and source-state digest, and stable receipt and export identities. The restore admission additionally binds an immutable pre-restore plan identity and digest available before execution. That plan covers the distinct isolated target, intended immutable backup/object version, source state, quarantine denominator, verification denominator, and objective policy; it never depends on a final restore receipt or other post-execution evidence. The backup admission and its verification must both occur strictly before `snapshot_at`; the restore admission and verification must both occur strictly before restore starts. Equality, later issuance, or later verification is post-hoc and fails closed. The admission is supplied independently and authenticated with the same pinned trust anchor under the separate domain `fawxzzy-platform:execution-gate-admission:v1`; a receipt field, `CURRENT` label, or recomputable digest cannot admit execution. The committed operational gates remain `BLOCKED` until a separately authorized packet produces this evidence.

The completed restore outcome is authenticated separately from its pre-restore admission. A closed result binds the admitted plan identity to the exact final restore receipt, including target, quarantine, parity, timeline, measured objectives, and deletion hold, then verifies a deterministic Ed25519 signature under `fawxzzy-platform:restore-outcome-attestation:v1`. The result must be current, no earlier than data-plane readiness, no later than the injected action-time clock, independently receipted, and signed by the source-owned trust anchor. Plan drift, swapped target/export, post-hoc admission, replayed or self-correlated evidence, an unauthenticated outcome, or any signature/key/verifier substitution fails closed.

The history readback binds the exact source project, schedule and retention policy, complete UTC month-to-observation window, entry count, canonical history digest, and a gap-free predecessor chain. Every accepted entry binds completion and acceptance time, retained immutable object identity, retention deadline, external immutable readback digest, and canonical entry digest. Caller array order and `completed_at` do not decide monthly retention. The validator independently proves both timelines: every `accepted_at` must be valid and unique, sorted acceptance observations may be at most six hours from the UTC month boundary, each other, and the observation time; completion timestamps must be valid, unique, strictly increasing in manifest order, within the same selected UTC month, and independently satisfy the same month-start, adjacent, and observation-tail bounds. The exact six-hour boundaries pass; any larger completion or acceptance gap fails. The earliest acceptance receives 400-day retention from completion and every other export receives at least 35 days. Duplicate, malformed, non-monotonic, cross-month, or delayed-substitution chronology fails closed. Missing, self-reported, circular, incomplete, stale, future-dated, cadence-gapped, project/policy-mismatched, retention-shortened, or unverifiable predecessor evidence also fails closed, so neither a fabricated prior entry nor delayed acceptance of an old export can waive retention or simulate completed export cadence.

The Object Lock readback is also supplied independently. It is current only when its observation timestamp is within the approved freshness window and after backup completion. The validator binds its provider, region, destination reference, immutable object version, ciphertext digest, compliance lock, non-mutable state, and retention deadline to the export. A second provider-bound attestation reduces the bucket reference and exact object identity to canonical digests, binds the readback and receipt manifests, and repeats the exact immutable version, ciphertext, mode, retention, and observation. That attestation is authenticated by the pinned trust anchor under `fawxzzy-platform:object-lock-provider-attestation:v1`. Provider, bucket, object, version, digest, retention, timestamp, receipt, or readback substitution; self-correlation; an untrusted signer; or an invalid signature fails closed. Missing, stale, ambiguous, unlocked, mutable, mismatched, shortened, malformed, or digest-only readback never establishes immutability.
