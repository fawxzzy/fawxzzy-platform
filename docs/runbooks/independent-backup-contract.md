...
Receipt validation also requires independently authenticated execution admission. `BACKUP_CURRENT` requires the source-owned `backup_generation_or_upload` gate at version `1.0.0` and `backup_receipt` scope. `RESTORE_REHEARSED` requires that admission plus `restore_rehearsal` at version `1.0.0` and `restore_receipt` scope. Each admission binds the canonical source policy artifact digest, the execution artifact identity derived from source commit, migration ledger, and tool versions, the project, receipt manifest, exact completed export, and—when applicable—the restore target and restore-receipt digest. Its issued/expiry interval and verifier time must be fresh and must precede the admitted execution boundary. The admission is supplied independently and authenticated with the same pinned trust anchor under the separate domain `fawxzzy-platform:execution-gate-admission:v1`; a receipt field, `CURRENT` label, or recomputable digest cannot admit execution. The committed operational gates remain `BLOCKED` until a separately authorized packet produces this evidence.
