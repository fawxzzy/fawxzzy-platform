...
8. a separate, closed, current accepted-exports manifest whose canonical digest matches the receipt, plus an authenticated external readback that proves the complete month-to-observation history, exact project and policy denominator, chained immutable identities, acceptance chronology, and retained object versions; retention is 35 days generally and 400 days for the first accepted monthly export;
...
The accepted-exports manifest and its closed authenticated readback are supplied independently from the receipt at validation time. The readback binds its own external authentication receipt, exact source project, schedule and retention policy, complete UTC month-to-observation window, entry count, canonical history digest, and a gap-free predecessor chain. Every accepted entry binds completion and acceptance time, retained immutable object identity, retention deadline, external immutable readback digest, and canonical entry digest. The current export must be the latest entry. The first entry must prove 400-day retention; later entries must prove at least 35 days. Missing, self-reported, circular, incomplete, stale, future-dated, duplicate, ambiguous, project/policy-mismatched, retention-shortened, or unverifiable predecessor evidence fails closed, so a fabricated prior entry cannot waive first-of-month retention.
