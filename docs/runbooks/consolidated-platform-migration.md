...
The Mazer adapter is the first closed product adapter. Its accepted denominator is exactly four provider-canonical relations. `public.mazer_profiles` becomes a private profile-seed commitment that is consumed atomically by server-side Mazer activation; it is never preinserted and the request never accepts a user ID. `public.mazer_progression_states` is authoritative and rekeys through the immutable source identity ledger. `public.mazer_ai_progression_states` is an indexed mirror excluded from transport and rebuilt after authoritative progression parity. `public.mazer_cycle_receipts` is append-only history whose source identity is preserved rather than regenerated by a target default. Revision and timestamp fields are accelerators only; S0, S1, and S2 still require complete primary-key and canonical-row comparisons.

Mazer insert/update order is canonical Auth mapping, pending membership, atomic profile-seed activation, progression, cycle receipts, and only then the AI mirror rebuild. Deletes remain child-first and tombstone-driven; profile and membership history is preserved. Conflicts quarantine without overwrite. Fitness and DiscordOS adapters remain `BLOCKED`, so this source-ready Mazer contract does not admit application-data execution.
