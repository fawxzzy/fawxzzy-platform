...
The Mazer adapter is the first closed product adapter. Its accepted denominator is exactly four provider-canonical relations. `public.mazer_profiles` becomes a private profile-seed commitment that is consumed atomically by server-side Mazer activation; it is never preinserted and the request never accepts a user ID. `public.mazer_progression_states` is authoritative and rekeys through the immutable source identity ledger. `public.mazer_ai_progression_states` is also authoritative: every runner key and every source column, including independent state and summary payloads, must survive complete key/digest parity and CAS transport. It may never be derived from human progression. `public.mazer_cycle_receipts` is append-only history whose source identity is preserved rather than regenerated by a target default. Revision and timestamp fields are accelerators only; S0, S1, and S2 still require complete primary-key and canonical-row comparisons.
...
Billing, entitlements, follow-up jobs, reporting, and Discord-named relations owned by the Fitness source are closed held classes. They may not be silently transported or dropped. Billing waits for verified billing provenance; operational jobs and Discord effects stay quarantined; Discord/provider/security-token identities cannot become shared human identity evidence. The historical `discord_bug_reports` identity is explicitly excluded as superseded.

The DiscordOS adapter binds provider-canonical history at 17 migrations even though the current default branch discovers 11. Current Git paths or bytes cannot substitute for the accepted package. Its exact 10-relation denominator is five authoritative mutable relations, four append-only history relations, and one held operational relation. Nine relations may enter a separately authorized transport rehearsal only after complete S0/S1/S2 key/digest comparison and identity/overlap quarantine; `discordos.discord_update_drafts` remains held with no transport. Parent-first inserts and child-first tombstones preserve the feedback-report and music-session dependency graphs, CAS conflicts quarantine instead of overwrite, and rollback uses the append-only journal in reverse dependency order.

DiscordOS is operational-only: no human activation, profile, entitlement, or membership creation is admitted. Discord IDs, fingerprints, usernames, labels, and snapshots never identify or authorize a human. Only an exact accepted source-identity-ledger mapping may permit rekeying; missing, duplicate, or contradictory evidence quarantines the record and leaves any existing membership pending. Synthetic proof rows are excluded and Fitness-semantic overlaps are quarantined without automatic merge.

The adapter holds every public RPC plus scheduler, `pg_cron`, `pg_net`, network-helper, Edge, credential, alias, provider-link, webhook, moderation-action, and Discord API write surface. The active source remains active. The inert package remains exactly 10 declared relations, 9 emitted relations, 1 held relation, 1 emitted helper function, 5 triggers, 22 indexes, and zero extension/data/cron/network effects. Mazer, Fitness, and DiscordOS are all source-ready, but application-data execution and all provider gates remain blocked.
...
- DiscordOS application-data adapter source contract: `SOURCE_READY`
