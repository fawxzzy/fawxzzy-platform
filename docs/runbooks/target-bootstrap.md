...
7. Confirm the generic application-data gate plus Mazer and Fitness adapters remain `SOURCE_READY` / `EXECUTION_BLOCKED` with `apply_admitted=false`, DiscordOS remains blocked, Fitness binds exactly 101 accepted migrations and 27 relations, the public receipt is redacted, and the accepted 122-migration package is unchanged.
