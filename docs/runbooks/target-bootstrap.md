...
The Data API containment postimage is required but not current. Contract v1.4.0 records the exact current preimage as enabled, Exposed schemas `[graphql_public, public]`, Extra search path `[public, extensions]`, automatic exposure off, tables 0/0, functions 1/0, and views `UNKNOWN/UNKNOWN`. The required postimage is disabled, `[]`, `[extensions]`, and off. Two earlier authorized Overview Save attempts did not persist; Settings Save attempts, rollback Saves, and persisted provider mutations are all zero. The final state equals the preimage. Sanitized Support case `SU-425819` is current, while case status and response timestamp remain `UNKNOWN`. Support observed the Data API enabled and requested one reproduction plus sanitized error-visual and network-activity-visual diagnostics. Provider defect, root cause, role change, alternate endpoint, and paid action remain `UNKNOWN`; no Dashboard case inbox exists.
...
6. Confirm Data API gate v1.4.0 preserves the exact current preimage, required postimage, earlier failed-Save accounting, Support case `SU-425819`, `UNKNOWN` case status/response timestamp/causes, FP-MAN-047's corrected-question plus canonical-answer envelope binding, the separately labeled answer-text digest, the consumed 1/1 attempt, both exact terminal receipt identities, `PREINTERACTION_LEDGER_VALIDATION_FAILURE` / `NO_SAVE_CONFIRMED`, retry forbidden, zero Dashboard Saves/readbacks/rollback Saves/persisted mutations for the terminal attempt, FP-MAN-037's rejected-collision and zero-authority record, the terminal blocked provider-execution boundary, required diagnostic redaction, blocked apply admission, and `UNKNOWN` view/probe fields without conflation or promotion. The terminal containment class remains `CONTAINABLE_WITH_HARD_GATES`.
