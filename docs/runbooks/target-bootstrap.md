...
The Data API containment postimage is required but not current. Contract v1.3.0 records the exact current preimage as enabled, Exposed schemas `[graphql_public, public]`, Extra search path `[public, extensions]`, automatic exposure off, tables 0/0, functions 1/0, and views `UNKNOWN/UNKNOWN`. The required postimage is disabled, `[]`, `[extensions]`, and off. Two authorized Overview Save attempts did not persist; Settings Save attempts, rollback Saves, and persisted provider mutations are all zero. The final state equals the preimage. Sanitized Support case `SU-425819` is current, while case status and response timestamp remain `UNKNOWN`. Support observed the Data API enabled and requested one reproduction plus sanitized error-visual and network-activity-visual diagnostics. Provider defect, root cause, role change, alternate endpoint, and paid action remain `UNKNOWN`; no Dashboard case inbox exists.

Prior two-Save action authority is consumed. FP-MAN-047 binds corrected question `onv1_ed934a7382f5e52e6ceea9ea73011f9ff70a46d31bd6061a3dc7645946cad0df` and its payload digest to canonical answer envelope `onv1_2a47e6b7bfb21d11ffe4cf87a7091f8aafb2d75ffebf25b3914dd6c03d8bb570` and its independently recomputable payload digest. The verbatim answer text retains a separate one-way digest and is not envelope identity. This approves at most one guarded reproduction attempt and sanitized diagnostic capture as policy only. FP-MAN-037 is a rejected cross-project decision-ID collision with zero Data API authority and zero attempts; never use it for this gate. The corrected approval remains held behind a separately admitted provider-execution packet, so this source ratchet executes and admits zero Dashboard interactions. Do not attempt the reproduction from source authority alone. Any later receipt must serialize no screenshots, network bodies, headers, cookies, tokens, keys, request or response payloads, user data, PII, project secrets, or machine paths. Before any future activation, prove `public` absent, enforce the ordered maximum allowlist `platform_shared`, `discordos`, `mazer`, `fitness`, keep `graphql_public`, `public`, private, extension, and provider schemas never exposed, and require explicit grants, RLS, independent readback, negative REST/GraphQL/RPC probes, and exact reverse-order rollback. Setting mutation, bootstrap apply, and target apply remain blocked.
...
6. Confirm Data API gate v1.3.0 preserves the exact current preimage, required postimage, failed-attempt accounting, Support case `SU-425819`, `UNKNOWN` case status/response timestamp/causes, FP-MAN-047's corrected-question plus canonical-answer envelope binding, the separately labeled answer-text digest, one-attempt ceiling with zero attempts executed, FP-MAN-037's rejected-collision and zero-authority record, the separate blocked provider-execution boundary, required diagnostic redaction, blocked apply admission, and `UNKNOWN` view/probe fields without conflation or promotion. The terminal containment class remains `CONTAINABLE_WITH_HARD_GATES`.
