...
6. Confirm Data API gate v1.2.0 preserves the exact current preimage, required postimage, failed-attempt accounting, Support case `SU-425819`, `UNKNOWN` case status/response timestamp/causes, FP-MAN-037's one-attempt ceiling, the separate blocked provider-execution boundary, required diagnostic redaction, blocked apply admission, and `UNKNOWN` view/probe fields without conflation or promotion. The terminal containment class remains `CONTAINABLE_WITH_HARD_GATES`.
