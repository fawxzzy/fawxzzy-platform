...
export function validateSemantics(documents) {
  ...
  requireCondition(mazerAppDataAdapter.identity_and_activation?.profile_seed?.storage === 'PRIVATE_COMMITMENT' && mazerAppDataAdapter.identity_and_activation?.profile_seed?.lookup === 'SERVER_SIDE_IDENTITY_MAPPING' && mazerAppDataAdapter.identity_and_activation?.profile_seed?.consumption === 'ATOMIC_WITH_ACTIVATION' && mazerAppDataAdapter.identity_and_activation?.profile_seed?.direct_pre_activation_insert_allowed === false && mazerAppDataAdapter.identity_and_activation?.profile_seed?.exact_profile_parity_required === true, 'Mazer profile activation-seed boundary drift');
  const expectedMazerRelations = [
    ...
    ['public.mazer_ai_progression_states', 'mazer.mazer_ai_progression_states', 'DERIVED_INDEXED_MIRROR', ['user_id', 'runner_key'], 'EXCLUDE_AND_REBUILD'],
  ...
  requireCondition(mazerAppDataAdapter.relations?.[2]?.rebuild_source === 'mazer.mazer_progression_states.state' && mazerAppDataAdapter.relations?.[2]?.rebuild_timing === 'AFTER_AUTHORITATIVE_PARITY', 'Mazer AI mirror must remain excluded and rebuilt after authoritative parity');
  ...
  requireCondition(exactOrderedValues(mazerAppDataAdapter.dependency_ordering?.insert_update_order, ['auth_mapping', 'pending_membership', 'atomic_profile_seed_activation', 'progression', 'cycle_receipts', 'AI_mirror_rebuild']) && mazerAppDataAdapter.dependency_ordering?.foreign_key_cycles === 'NONE' && mazerAppDataAdapter.dependency_ordering?.external_effects === 'QUARANTINED', 'Mazer app data dependency ordering drift');
  ...
  requireCondition(exactOrderedValues(appDataAdaptersGate.required_order, ['mazer', 'fitness', 'discordos']) && exactOrderedValues(appDataAdaptersGate.source_ready, ['mazer']) && exactOrderedValues(appDataAdaptersGate.blocked, ['fitness', 'discordos']), 'app data adapter readiness denominator drift');
  requireCondition(appDataAdaptersGate.mazer_contract_path === 'contracts/v1/transport/mazer-app-data-adapter-contract.json' && appDataAdaptersGate.mazer_relation_count === 4 && appDataAdaptersGate.all_adapters_ready === false && appDataAdaptersGate.execution_lifecycle === 'EXECUTION_BLOCKED' && appDataAdaptersGate.apply_admitted === false, 'Mazer app data adapter migration gate drift');
  const provenance = migrationGate.provider_canonical_provenance;
  requireCondition(migrationGate.version === '1.2.0', 'migration gate version must remain 1.2.0');
