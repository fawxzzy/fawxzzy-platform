...
export function validateSemantics(documents) {
  ...
  requireCondition(exactOrderedValues(appDataAdaptersGate.required_order, ['mazer', 'fitness', 'discordos']) && exactOrderedValues(appDataAdaptersGate.source_ready, ['mazer', 'fitness']) && exactOrderedValues(appDataAdaptersGate.blocked, ['discordos']), 'app data adapter readiness denominator drift');
  requireCondition(appDataAdaptersGate.mazer_contract_path === 'contracts/v1/transport/mazer-app-data-adapter-contract.json' && appDataAdaptersGate.mazer_relation_count === 4 && appDataAdaptersGate.fitness_contract_path === 'contracts/v1/transport/fitness-app-data-adapter-contract.json' && appDataAdaptersGate.fitness_relation_count === 27 && appDataAdaptersGate.all_adapters_ready === false && appDataAdaptersGate.execution_lifecycle === 'EXECUTION_BLOCKED' && appDataAdaptersGate.apply_admitted === false, 'application-data adapter migration gate drift');
  ...
  requireCondition(migrationGate.version === '1.3.0', 'migration gate version must remain 1.3.0');
