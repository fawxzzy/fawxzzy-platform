  ...
  retry_authority: Object.freeze({
    ...
    rejected_decision_collisions: Object.freeze([
      Object.freeze({
        ...
        data_api_authority_granted: false,
    ...
    guarded_reproduction_terminal_outcome: Object.freeze({
      status: 'CURRENT',
      classification: 'PREINTERACTION_LEDGER_VALIDATION_FAILURE',
      result: 'NO_SAVE_CONFIRMED',
      attempt_consumption_event_id: 'onv1_6258aed05023737d6403a35dcf0867e873ab64513578d25713c9584c830e3836',
      terminal_receipt_event_id: 'onv1_6515ddefc604a92dcf4849395a0dfd19a191b1139891a233318636f5a81e683b',
      consume_before_interaction_order_proven: false,
      attempt_limit: 1,
      attempts_consumed: 1,
...
export function verifyProviderCanonicalProvenance({ gate, sourceManifest, migrationPackageSha256, governanceManifestSha256 }) {
  ...
  fail(failures, gate?.status === 'BLOCKED' && gate?.version === '1.6.0', 'provider-canonical migration gate must remain BLOCKED at version 1.6.0');
  fail(failures, canonicalJson(gate?.data_api_decision_binding) === canonicalJson(dataApiDecisionBindingV1), 'Data API manual decision binding drift');
...
export function verifyMazerAppDataAdapter({ adapter, gate }) {
  ...
  fail(failures, adapterGate.mazer_contract_path === 'contracts/v1/transport/mazer-app-data-adapter-contract.json' && adapterGate.mazer_relation_count === 4 && adapterGate.fitness_contract_path === 'contracts/v1/transport/fitness-app-data-adapter-contract.json' && adapterGate.fitness_relation_count === 27 && adapterGate.discordos_contract_path === 'contracts/v1/transport/discordos-app-data-adapter-contract.json' && adapterGate.discordos_relation_count === 10 && adapterGate.discordos_transport_ready === false && adapterGate.discordos_block_reason === 'MUSIC_SESH_INDEPENDENT_DOMAIN_REQUIRES_SEPARATE_TARGET_AND_REGENERATED_ARTIFACT' && adapterGate.all_adapters_ready === true && adapterGate.execution_lifecycle === 'EXECUTION_BLOCKED' && adapterGate.apply_admitted === false, 'Mazer app data adapter execution gate drift');
...
export function verifyFitnessAppDataAdapter({ adapter, gate }) {
  ...
  fail(failures, adapterGate.fitness_contract_path === 'contracts/v1/transport/fitness-app-data-adapter-contract.json' && adapterGate.fitness_relation_count === 27 && adapterGate.discordos_contract_path === 'contracts/v1/transport/discordos-app-data-adapter-contract.json' && adapterGate.discordos_relation_count === 10 && adapterGate.discordos_transport_ready === false && adapterGate.discordos_block_reason === 'MUSIC_SESH_INDEPENDENT_DOMAIN_REQUIRES_SEPARATE_TARGET_AND_REGENERATED_ARTIFACT' && adapterGate.all_adapters_ready === true && adapterGate.execution_lifecycle === 'EXECUTION_BLOCKED' && adapterGate.apply_admitted === false, 'Fitness app data adapter execution gate drift');
...
export function verifyDiscordosAppDataAdapter({ adapter, gate }) {
  ...
  const expectedRelationCore = [
    ...
    ['discordos.discordos_music_sesh_sessions', null, 'HELD_INDEPENDENT_DOMAIN', ['session_id'], 'HOLD_INDEPENDENT_DOMAIN'],
    ['discordos.discordos_music_sesh_queue_items', null, 'HELD_INDEPENDENT_DOMAIN', ['queue_item_id'], 'HOLD_INDEPENDENT_DOMAIN'],
    ['discordos.discordos_music_sesh_votes', null, 'HELD_INDEPENDENT_DOMAIN', ['vote_id'], 'HOLD_INDEPENDENT_DOMAIN'],
  ...
  fail(failures, inert.emitted_extension_count === 0 && inert.emitted_data_effect_count === 0 && inert.emitted_cron_effect_count === 0 && inert.emitted_network_effect_count === 0 && inert.music_sesh_relation_count === 3 && inert.music_sesh_artifact_status === 'INCOMPATIBLE_UNADMITTED' && inert.music_sesh_artifact_regeneration === 'REQUIRED_SEPARATE_AUTHORITY' && inert.generated_artifact_changes_admitted === false, 'DiscordOS inert external-effect or generated-artifact boundary drift');
  ...
  fail(failures, canonicalJson(adapter?.classification_counts) === canonicalJson({ authoritative_state: 2, authoritative_append_only_history: 4, held_operational_external_effect: 1, held_independent_domain: 3, transported: 6, held: 4, total: 10 }), 'DiscordOS relation classification count drift');
  fail(failures, canonicalJson(adapter?.dependency_ordering?.insert_update_order) === canonicalJson(['discordos.discord_feedback_reports', 'discordos.discord_feedback_audit_events', 'discordos.discord_feedback_completion_reviews', 'discordos.runtime_health_cron_runs', 'discordos.discordos_board_cards', 'discordos.discordos_moderation_audit_log']), 'DiscordOS insert/update dependency order drift');
  fail(failures, canonicalJson(adapter?.dependency_ordering?.delete_order) === canonicalJson(['discordos.discord_feedback_completion_reviews', 'discordos.discord_feedback_audit_events', 'discordos.discord_feedback_reports', 'discordos.discordos_moderation_audit_log', 'discordos.discordos_board_cards', 'discordos.runtime_health_cron_runs']) && canonicalJson(adapter?.dependency_ordering?.held_relations_excluded_from_order) === canonicalJson(['discordos.discordos_music_sesh_sessions', 'discordos.discordos_music_sesh_queue_items', 'discordos.discordos_music_sesh_votes', 'discordos.discord_update_drafts']) && adapter?.dependency_ordering?.foreign_key_cycles === 'NONE' && adapter?.dependency_ordering?.external_effects === 'QUARANTINED', 'DiscordOS delete order, held relation, or cycle boundary drift');
  ...
  fail(failures, adapterGate.discordos_contract_path === 'contracts/v1/transport/discordos-app-data-adapter-contract.json' && adapterGate.discordos_relation_count === 10 && adapterGate.discordos_transport_ready === false && adapterGate.discordos_block_reason === 'MUSIC_SESH_INDEPENDENT_DOMAIN_REQUIRES_SEPARATE_TARGET_AND_REGENERATED_ARTIFACT' && adapterGate.all_adapters_ready === true && adapterGate.execution_lifecycle === 'EXECUTION_BLOCKED' && adapterGate.apply_admitted === false, 'DiscordOS app data adapter execution gate drift');
...
export function verifyHeldControlPlaneContracts(config, namespacePlan) {
  ...
  fail(failures, manualDecision.answer_event_id === dataApiDecisionBindingV1.answer_event_id && manualDecision.answer_payload_sha256 === dataApiDecisionBindingV1.answer_payload_sha256, 'guarded reproduction canonical answer event or digest drift');
  fail(failures, manualDecision.answer_text_sha256 === dataApiDecisionBindingV1.answer_text_sha256, 'guarded reproduction answer-text digest drift');
